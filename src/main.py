...
from utils.utils_func import make_dataset_path
...
def main(config_path:Config, args:ArgumentParser):
    ...
    if (args.cont and args.mode == 'train') or args.mode != 'train':
        ...
        if not os.path.exists(base_path + 'data'):
            raise FileNotFoundError('data folder does not exist, change name "data_sample" to "data"')

        # check tokenizer file
        if not os.path.isdir(base_path+'data/kowikitext/tokenizer/vocab_'+str(config.vocab_size)):
            print('You must make vocab and tokenizer first..')
            sys.exit()
        config.tokenizer_path = base_path+'data/kowikitext/tokenizer/vocab_'+str(config.vocab_size)+'/vocab.txt'
